...
# Check both possible policy names (Observability or agentcore-permissions)
HAS_XRAY=$(aws iam get-role-policy --role-name agentis-agentcore-role \
  --policy-name Observability \
  --query 'PolicyDocument' --output json 2>/dev/null | grep -c "xray:PutTraceSegments" || \
  aws iam get-role-policy --role-name agentis-agentcore-role \
...
# Sample one of our agentis eval log groups to verify filter exists
SAMPLE_LG=$(aws logs describe-log-groups \
  --log-group-name-prefix /aws/bedrock-agentcore/evaluations/results/eval_agentis \
