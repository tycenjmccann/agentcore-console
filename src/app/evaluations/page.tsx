...
export default function EvaluationsPage() {
  ...
  const fetchAgents = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      const allEnabled = data.length > 0 && data.every((a: AgentEvalConfig) => a.enabled);
  ...
  const updateAgent = useCallback(
    async (agentId: string, updates: Partial<Pick<AgentEvalConfig, "enabled" | "sampleRate" | "batchSize">>) => {
      ...
      try {
        ...
        setAgents((prev) =>
          prev.map((a) => (a.agentId === agentId ? { ...a, ...updated } : a))
        );
      } catch (err) {
        const msg = err instanceof Error ? err.message : "Unknown error";
  ...
  const handleFlush = useCallback(
    async (agentId: string) => {
      ...
      try {
        ...
        setAgents((prev) =>
          prev.map((a) => (a.agentId === agentId ? { ...a, bufferCount: 0 } : a))
        );
      } catch (err) {
        const msg = err instanceof Error ? err.message : "Unknown error";
  ...
  // Buffer color helper
  function getBufferColor(count: number, batchSize: number): string {
    if (count === 0) return "bg-green-400/20 text-green-400 border-green-400/30";
...
interface AgentRowProps {
  ...
  getBufferColor: (count: number, batchSize: number) => string;
}
    ...
    >
      ...
      <td className="px-4 py-4 text-center">
        <span
          className={cn(
            ...
            getBufferColor(agent.bufferCount, agent.batchSize)
          )}
        >
          {agent.bufferCount}/{agent.batchSize}
        </span>
      </td>
      ...
      <td className="px-4 py-4 text-center">
        <button
          ...
          disabled={agent.bufferCount === 0 || isFlushing || !agent.enabled}
          className={cn(
            "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors",
            agent.bufferCount === 0 || !agent.enabled
