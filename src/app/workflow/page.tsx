...
export default function WorkflowPage() {
  ...
  const fetchWorkflows = useCallback(async () => {
    try {
      ...
      const list: WorkflowEntry[] = (data.workflows || []).map(
        (w: WorkflowState) => ({
          id: w.id,
          phase: w.phase,
          epicId: w.epicId,
        ...
        {showIntake ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <p className="text-sm text-[var(--color-text-muted)] max-w-md mb-4">
              Choose a past run from the sidebar to view its pipeline state, or
              create a new workflow to watch agents work in real-time.
            </p>
            <button
